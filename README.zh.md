...
| `better_edit_delete` | 删除行 — 支持单行、范围删除和 JSON 数组批量删除。 |
| `better_edit_batch` | 批量编辑文件（支持多文件），一次调用完成多处修改。操作自动从下往上排序，无需担心行号偏移。 |
| `better_edit_write` | 写入原始内容到文件。支持单文件 `{"file","content"}` 和多文件 `{"files":[...]}` 格式。当标准 JSON 解析失败时（如反引号、`${}` 未转义），自动启用状态机降级提取。 |
| `better_edit_function_range` | 传入文件路径和行号，返回该行所在函数的起止行号（基于花括号计数，支持字符串/注释感知）。 |
...
- **JSON 降级解析**：AI 生成的 JSON 格式有误时（未转义引号、反引号、`${}`），`write` 工具自动降级为字符级状态机提取 `file` 和 `content` 字段。
