...
> ~~⚠️ **中文用户请注意**：如果你使用中文大模型（如 Kimi、文心一言等），请在 MCP 配置中显式添加 `"args": ["--lang", "zh"]`，否则工具描述会回退为英文，增加模型理解和调用成本。~~ 根据最新的调查结果，暂时不再推荐中文大模型使用中文提示词，因为相同含义下的中文提示词的分词器会让 token 数量更长
...
- Session 状态桥接：`be-read` 返回 `viewed_code_id`，`be-replace` 可传入校验行数一致性。
