...
> 这是一个高性能的 MCP（Model Context Protocol）文件编辑工具集 - Go 原生实现，提供原子写入、批量编辑智能排序、函数范围自动检测。希望通过该工具减少大模型修改代码产生的 token 数量并且尽可能挽回大模型因为调用工具参数错误产生的各种问题
...
如果传入 `old`，工具会先校验当前文件内容是否与旧内容一致；不一致时会直接返回错误，而不是静默替换目标行范围。为了兼容不同客户端，服务端也接受 `old_text` 作为别名。

── 已知目标区间时，直接做最小范围的精确替换。

### `be-insert`

`be-insert` 用于在指定行后插入内容，`line=0` 时表示插入到文件开头。服务端也接受 `after_line` 作为同义参数。它是增量编辑里最直接的原语，既能减少对原有内容的扰动，也能和共享参数体系保持一致。

── 把新内容插到准确位置，同时尽量不扰动原文件。

### `be-delete`

`be-delete` 负责删除单行、行范围或 JSON 数组指定的多行。范围删除时，服务端同时接受 `start` / `end` 和 `start_line` / `end_line` 两种写法。它的设计偏向清理型修改，强调操作的可预测性，所以始终围绕“行”这个粒度来工作。
...
`be-write` 是原始写入工具，用来直接写入完整文件内容。它使用直接参数调用：

- 单文件：`{"file":"...","content":"..."}`
- 多文件：`{"files":[{"file":"...","content":"..."}]}`

而且在标准 JSON 解析失败时，会自动切换到状态机式的降级提取逻辑，尽量保证 AI 生成内容也能顺利落盘。
