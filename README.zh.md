...
| `be-replace` | 替换文件中指定行范围的内容。支持共享的 `target` 和 `preview` 参数。 |
| `be-insert` | 在指定行后插入内容（`line=0` 表示插入到文件开头）。支持共享的 `target` 和 `preview` 参数。 |
| `be-delete` | 删除行 — 支持单行、范围删除和 JSON 数组批量删除。支持共享的 `target` 和 `preview` 参数。 |
| `be-batch` | 批量编辑文件（支持多文件），一次调用完成多处修改。操作自动从下往上排序，无需担心行号偏移。支持共享的 `preview` 参数。 |
| `be-write` | 写入原始内容到文件。支持单文件 `{"file","content"}` 和多文件 `{"files":[...]}` 格式。当标准 JSON 解析失败时（如反引号、`${}` 未转义），自动启用状态机降级提取。支持共享的 `preview` 参数。 |
| `be-func-range` | 传入文件路径和行号，返回该行所在 `{}` 块/函数的起止行号（基于花括号计数，支持字符串/注释感知）。 |
| `be-tag-range` | 传入文件路径和行号，返回该行所在 XML/HTML/Vue tag 配对范围。 |
