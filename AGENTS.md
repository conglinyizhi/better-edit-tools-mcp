...
提交信息使用约定式提交（Conventional Commits），例如 `feat: ...`、`fix: ...`、`docs: ...`、`chore: ...`、`refactor: ...`、`test: ...`、`perf: ...`。这样 GitHub Actions 可以自动按类型生成 changelog。commit message 部分使用中文，如果某个 commit 涉及到了一个 github issue 编号，则在末尾写入 close #xx 让 github 自动关闭这个 issue

建议统一使用下面的格式：

```text
feat(parser)!: 新增xx特性，同时xx特性被废弃
fix(server): 修复xx导致的xx不符合预期行为
docs: 更新文档
refactor(edit): 调整xxxx文件的格式
test(release): 调整测试链路
```
